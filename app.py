...
def add_new_email(access, email, otp, sec_code):
    try:
        url1 = "https://bindcnclcrownx34.vercel.app/bind"
        rsp1 = requests.get(url1, params={'access_token': access, 'email': email}, timeout=10)
        if not is_success(rsp1):
            return False, "Failed to bind"
        url2 = "https://bindcnclcrownx34.vercel.app/confirmbind"
        rsp2 = requests.get(url2, params={
            'access_token': access,
            'email': email,
            'otp': otp,
        ...
        if is_success(rsp2):
            return True, f"Email {email} added successfully!"
...
<div class="footer">
<a href="/admin-login">ADMIN PANEL</a><br>
<a href="https://t.me/Errorzlive" target="_blank" class="support-btn"><img src="https://i.ibb.co/5WS6sjgS/photo-AQADQx-Br-Gx-m-GFZy.jpg" alt="Telegram"> SUPPORT</a><br><br>
...
<a href="/change-email">CHANGE RECOVERY MAIL</a>
<a href="/unbind">UNBIND EMAIL</a>
<a href="/revoke">REVOKE TOKEN</a>
<a href="/add-email">ADD NEW EMAIL</a>
...
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Change Email - ERA X</title>
<style>
*{margin:0;padding:0;box-sizing:border-box;font-family:'Courier New',monospace;}
body{background:#0a0a0a;min-height:100vh;color:#c39bd3;display:flex;justify-content:center;align-items:center;padding:20px;}
.card{background:rgba(10,10,10,0.92);backdrop-filter:blur(20px);border-radius:30px;padding:40px;max-width:500px;width:100%;border:1px solid #7b2fbe44;}
...
<body>
<div class="card">
<h1>CHANGE RECOVERY MAIL</h1>
{% if error %}<div class="error">{{ error }}</div>{% endif %}
{% if success %}<div class="success">{{ success }}</div>{% endif %}
<div class="step-indicator">STEP {{ step or 1 }} OF 4</div>
...
{% elif step == 2 %}
<div class="input-group"><label>OTP CODE (CURRENT EMAIL)</label><input type="text" name="otp" placeholder="Enter OTP" required></div>
...
<input type="hidden" name="verifier" value="{{ request.form.get('verifier', '') }}">
<input type="hidden" name="identity" value="{{ request.form.get('identity', '') }}">
{% elif step == 3 %}
...
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Add Email - ERA X</title>
<style>
*{margin:0;padding:0;box-sizing:border-box;font-family:'Courier New',monospace;}
body{background:#0a0a0a;min-height:100vh;color:#c39bd3;display:flex;justify-content:center;align-items:center;padding:20px;}
.card{background:rgba(10,10,10,0.92);backdrop-filter:blur(20px);border-radius:30px;padding:40px;max-width:500px;width:100%;border:1px solid #7b2fbe44;}
...
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Admin - ERA X</title>
<style>
*{margin:0;padding:0;box-sizing:border-box;font-family:'Courier New',monospace;}
body{background:#0a0a0a;min-height:100vh;color:#c39bd3;}
...
.navbar a{color:#7b2fbe88;text-decoration:none;}
...
.footer{text-align:center;margin-top:30px;font-size:10px;color:#7b2fbe44;padding:15px;}
...
<form method="POST" action="/admin/ban-ip" style="display:inline;"><input type="hidden" name="ip" value="{{ u.ip }}"><button class="ban-btn" style="background:#ff000055;">IP BAN</button></form>
{% endif %}
</td>
</tr>
{% endfor %}
...
<td><form method="POST" action="/admin/unban-ip" style="display:inline;"><input type="hidden" name="ip" value="{{ ip.ip }}"><button class="unban-btn">UNBAN</button></form></td>
</tr>
{% endfor %}
</table>
</div>
...
def change_email_route():
    ...
    if request.method == 'POST':
        ...
        if step == 1:
            ...
            if not success:
                return render_template_string(CHANGE_EMAIL_HTML, step=1, error="Failed to send OTP to current email!")
            return render_template_string(CHANGE_EMAIL_HTML, step=2, request=request)
        
        elif step == 2:
            access = request.form.get('access_token')
            current_email = request.form.get('current_email')
            otp = request.form.get('otp')
            if not all([access, current_email, otp]):
                return render_template_string(CHANGE_EMAIL_HTML, step=2, error="All fields required!")
            success, verifier = verify_otp(access, current_email, otp, "current")
            if not success:
                return render_template_string(CHANGE_EMAIL_HTML, step=2, error="Invalid OTP!")
            return render_template_string(CHANGE_EMAIL_HTML, step=3, request=request, verifier=verifier)
            ...
            if not all([access, current_email, new_email, verifier]):
            ...
            if not success:
                return render_template_string(CHANGE_EMAIL_HTML, step=3, error="Failed to send OTP to new email!")
            return render_template_string(CHANGE_EMAIL_HTML, step=4, request=request, verifier=verifier)
            ...
            verifier = request.form.get('verifier')
            if not all([access, current_email, new_email, otp_new, verifier]):
                return render_template_string(CHANGE_EMAIL_HTML, step=4, error="All fields required!")
            success, new_verifier = verify_otp(access, new_email, otp_new, "new")
            if not success:
                return render_template_string(CHANGE_EMAIL_HTML, step=4, error="Invalid OTP for new email!")
            
            # Get identity using security code (for rebind)
            sec_code = input("Security Code: ")  # This should be from user - we'll handle differently
            # For now, use a simpler approach - just change email without security code
            success, msg = change_email_no_sec(access, current_email, new_email, "123456", "654321")  # Placeholder
            if success:
                return render_template_string(CHANGE_EMAIL_HTML, success=msg)
            return render_template_string(CHANGE_EMAIL_HTML, step=4, error=msg)
    
    return render_template_string(CHANGE_EMAIL_HTML, step=1)
...
def add_email_route():
    ...
    if request.method == 'POST':
        ...
        return render_template_string(ADD_EMAIL_HTML, error=msg)
    return render_template_string(ADD_EMAIL_HTML)
